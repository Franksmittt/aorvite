...
export const PACKAGE_TEMPLATES: PackageTemplate[] = [
  {
    ...
    steps: [
      ...
      { id: 'fb-strip-1', stepOrder: 2, taskName: 'Strip top cover above the grille — retain clips/fasteners; photo', requiresPhoto: true, skippable: false },
      { id: 'fb-strip-covers', stepOrder: 3, taskName: 'Remove 2 covers in front of the wheel arches (both sides) — photo of both covers', requiresPhoto: true, skippable: false },
      { id: 'fb-strip-2', stepOrder: 4, taskName: 'Remove screws from bottom of bumper + clips from wheel arches (both sides); photo', requiresPhoto: true, skippable: false },
      { id: 'fb-strip-harness', stepOrder: 5, taskName: 'Bumper loose — unplug wiring harness: light plug + 1 PDC each side; photo harness L/R', requiresPhoto: true, skippable: false, photoMode: 'multi', minPhotos: 2 },
      { id: 'fb-strip-bumper', stepOrder: 6, taskName: 'Front bumper off on the ground — 2 photos', requiresPhoto: true, skippable: false, photoMode: 'multi', minPhotos: 2 },
      { id: 'fb-strip-mid-cover', stepOrder: 7, taskName: 'Remove plastic cover between bash plate and crash bar (steel impact beam); photo', requiresPhoto: true, skippable: false },
      { id: 'fb-strip-crashbar', stepOrder: 8, taskName: 'Remove steel crash bar + its 2 plastic covers — photo of covers and crash bar', requiresPhoto: true, skippable: false, photoMode: 'multi', minPhotos: 3 },
      { id: 'fb-strip-bash', stepOrder: 9, taskName: 'Remove bash plate (skid plate) — bag bolts; photo', requiresPhoto: true, skippable: false },
      { id: 'fb-2', stepOrder: 10, taskName: 'Fully stripped front — underneath + front-left + front + front-right; hardware bagged', requiresPhoto: true, skippable: false, photoMode: 'multi', minPhotos: 4 },
      { id: 'fb-unpack', stepOrder: 11, taskName: 'Unpack new bumper from wrapping — inspect for damage; photo', requiresPhoto: true, skippable: false },
      { id: 'fb-3', stepOrder: 12, taskName: 'Front parking sensors transferred and seated correctly (L/R)', requiresPhoto: false, skippable: true },
      { id: 'fb-4', stepOrder: 13, taskName: 'Indicators / fog lights / DRL mounted with correct fasteners', requiresPhoto: false, skippable: true },
      { id: 'fb-5', stepOrder: 14, taskName: 'Spotlights / light bar mounts fitted (if on job)', requiresPhoto: false, skippable: true },
      { id: 'fb-6', stepOrder: 15, taskName: 'Front camera / radar remounted (if fitted)', requiresPhoto: false, skippable: true },
