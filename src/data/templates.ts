...
export const PACKAGE_TEMPLATES: PackageTemplate[] = [
  {
    ...
    steps: [
      ...
      // Strip order (Hilux): top cover → wheel-arch covers → bottom screws/clips → unplug harness → bumper off
      ...
      { id: 'fb-2', stepOrder: 7, taskName: 'Photo of stripped front chassis; all clips, bolts, and washers accounted for', requiresPhoto: true, skippable: false },
      { id: 'fb-3', stepOrder: 8, taskName: 'Front parking sensors transferred and seated correctly (L/R)', requiresPhoto: false, skippable: true },
      { id: 'fb-4', stepOrder: 9, taskName: 'Indicators / fog lights / DRL mounted with correct fasteners', requiresPhoto: false, skippable: true },
      { id: 'fb-5', stepOrder: 10, taskName: 'Spotlights / light bar mounts fitted (if on job)', requiresPhoto: false, skippable: true },
      { id: 'fb-6', stepOrder: 11, taskName: 'Front camera / radar remounted (if fitted)', requiresPhoto: false, skippable: true },
